import { getPublishedRecipes, getAllCategories, getRecipeById } from '@/lib/recipe';
...
export default async function RecipesPage({
  ...
  // 过滤掉获取失败的菜谱
  const validRecipes = filteredRecipes.filter(recipe => recipe !== null) as RecipeWithDetails[];
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {validRecipes.map((recipe) => (
                <div key={recipe.id} className="transform hover:-translate-y-1 transition-all duration-300">
            ...
            {validRecipes.length > 9 && (
              <div className="mt-16 text-center">
                <p className="text-sm text-muted-foreground mb-4">
                  显示 {validRecipes.length} 个菜谱中的 {Math.min(validRecipes.length, 9)} 个
                </p>
