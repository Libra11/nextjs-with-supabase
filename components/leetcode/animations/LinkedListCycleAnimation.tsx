...
type StepPhase = "init" | "move" | "collision" | "no-cycle" | "done";
...
const buildCycleSteps = (values: number[], pos: number): StepState[] => {
  ...
  while (
    ...
    if (slow !== null && fast !== null && slow === fast) {
      pushStep(
        "collision",
        `slow 与 fast 相遇于 ${formatNodeLabel(values, slow)}，说明链表存在环。`,
        {
          fastMid: fastStepOne,
        },
      );
  ...
  if (foundCycle) {
    pushStep("done", "检测完成，返回 true。");
    return steps;
  }

  const reason =
    iteration >= MAX_ITERATIONS
      ? `超过最大迭代次数 ${MAX_ITERATIONS} 次仍未相遇，视为无环（防止无限循环）。`
      : fast === null
        ? "fast 指针移动到 null，说明链表存在尾节点，判定为无环。"
        : `fast 指针的下一节点为 null（${formatNodeLabel(values, fast)} -> null），说明链表无环。`;

  pushStep("no-cycle", reason);
  pushStep("done", "检测完成，返回 false。");
...
const buildAnimationData = (values: number[], pos: number): AnimationData => {
  ...
  return {
    ...
    pos: cappedPos,
    steps: buildCycleSteps(sanitizedValues, cappedPos),
  };
};

export default function LinkedListCycleAnimation() {
  ...
  return (
    <div className="w-full min-h-[520px] bg-gradient-to-br from-background to-muted/20 rounded-lg p-6 flex flex-col">
      <div className="text-center mb-6">
        <h3 className="text-xl font-bold">环形链表检测 - 快慢指针演示</h3>
        <p className="text-sm text-muted-foreground mt-1">
          使用 Floyd 判圈算法，观察 slow 与 fast 指针如何移动并判断链表是否存在环。
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.4fr_1fr] gap-6 flex-grow">
        <div className="bg-card/80 border border-border/60 rounded-xl p-5 flex flex-col">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6 text-xs text-muted-foreground">
                ...
                <div className="flex items-stretch gap-4 min-w-[360px]">
                  {values.map((value, idx) => {
                    ...
                    const nodeClass = cn(
                      ...
                      isSlow && isFast
                        ? "border-fuchsia-500 text-fuchsia-600 bg-fuchsia-500/10 shadow-fuchsia-500/20"
                        : isFast
                          ? "border-indigo-500 text-indigo-600 bg-indigo-500/10 shadow-indigo-500/20"
                          : isSlow
                            ? "border-primary text-primary bg-primary/10 shadow-primary/20"
                            : isCycleEntry
                              ? "border-amber-500/60 text-amber-600 bg-amber-500/10"
                              : isSlowTrail && isFastTrail
                                ? "border-sky-500/60 text-sky-600 bg-sky-500/10"
                                : isSlowTrail
                                  ? "border-primary/50 text-primary bg-primary/5"
                                  : isFastTrail || isFastMid
                                    ? "border-indigo-300 text-indigo-500 bg-indigo-500/5"
                                    : "border-border/70 text-foreground bg-card/70",
                    );

                    return (
                      <motion.div key={`${value}-${idx}`} layout className={nodeClass}>
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-2 text-[11px] uppercase tracking-wide">
                          {isSlow && (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 border border-primary/40 text-primary">
                              <Turtle className="h-3.5 w-3.5" />
                              slow
                            </span>
                          )}
                          {isFast && (
                            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-indigo-500/10 border border-indigo-500/40 text-indigo-600">
                              <Rabbit className="h-3.5 w-3.5" />
                              fast
                            </span>
                          )}
                          {!isFast && isFastMid && (
                            <span className="px-2 py-0.5 rounded-full bg-indigo-500/5 border border-indigo-500/30 text-indigo-500">
                              fast 路过
                            </span>
                          )}
                          {!isSlow && !isFast && isSlowTrail && !isFastTrail && (
                            <span className="px-2 py-0.5 rounded-full bg-primary/5 border border-primary/30 text-primary">
                              slow 轨迹
                            </span>
                          )}
                          {!isSlow && !isFast && isFastTrail && !isSlowTrail && (
                            <span className="px-2 py-0.5 rounded-full bg-indigo-500/5 border border-indigo-500/30 text-indigo-500">
                              fast 轨迹
                            </span>
                          )}
                          {!isSlow && !isFast && isSlowTrail && isFastTrail && (
                            <span className="px-2 py-0.5 rounded-full bg-sky-500/5 border border-sky-500/30 text-sky-600">
                              slow & fast
                            </span>
                          )}
                        </div>
